...
# Spark imports kept for potential future use with data_engg module
...
def main(input_path=None, review_path=None, search_id=None):
    ...
    Uses ThreadPoolExecutor for true parallel API calls.
    ...
    if input_path:
        ...
        # Process products in parallel using threads (max 5 concurrent API calls)
        results = []
        with ThreadPoolExecutor(max_workers=5) as executor:
            # Submit all tasks
            future_to_product = {
                executor.submit(process_product, row): row['name']
            ...
            for i, future in enumerate(as_completed(future_to_product), 1):
                ...
                print(f"[{i}/{len(grouped)}] Completed: {product_name}")
